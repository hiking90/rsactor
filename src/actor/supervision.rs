...
use super::{Context, SupervisionEvent, ActorId};

/// A supervision tree
#[derive(Default, Debug)]
pub(crate) struct SupervisionTree {
    children: Mutex<Option<HashMap<ActorId, Context>>>,
    supervisor: Mutex<Option<Context>>,
    #[cfg(feature = "monitors")]
    monitors: Mutex<Option<HashMap<ActorId, Context>>>,
}

impl SupervisionTree {
    /// Push a child into the tere
    pub(crate) fn insert_child(&self, child: Context) {
    ...
    pub(crate) fn set_supervisor(&self, parent: Context) {
    ...
    pub(crate) fn try_get_supervisor(&self) -> Option<Context> {
        self.supervisor.lock().unwrap().clone()
    }

    /// Set a monitor of this supervision tree
    #[cfg(feature = "monitors")]
    pub(crate) fn set_monitor(&self, who: Context) {
    ...
    pub(crate) fn get_children(&self) -> Vec<Context> {
